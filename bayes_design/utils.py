...
def get_fixed_position_mask_from_range(fixed_position_list, seq_len):
    """Function that generates a fixed position mask from a list of fixed positions.

    Args:
        fixed_position_list (list): list of fixed positions. These can be integers or range strings.
            i.e. [1, '5', '6-10']
        seq_len (int): length of the sequence
    """
    fixed_position_mask = np.zeros(seq_len)
    # Preserve fixed positions
    for pos in fixed_position_list:
        if "-" in pos:
            start, end = pos.split("-")
            fixed_position_mask[int(start) - 1 : int(end)] = 1.0
        else:
            # -1 because residues are 1-indexed
            fixed_position_mask[int(pos) - 1] = 1.0
    return fixed_position_mask
